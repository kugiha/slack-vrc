...
def online():
    ...
    for f in friends:
        ...
        if f.get('instance_type') == 'private':
            ...
            reply += '*{}* _in {}_{}'.format(
                f['name'], f['world_name'],
                get_instanceStatus_emoji(f['instance_type']))
    return reply
...
def online_grouped():
    ...
    users_by_world = sorted(
        users_by_world.items(),
    ...
    for item in users_by_world:
        ...
        reply = reply[:-2]
        reply += ' _in {}_ ({}/{}) {}\n'.format(
            users[0]['world_name'], users[0]['instance_users_count'],
            users[0]['world_capacity'],
            get_instanceStatus_emoji(users[0]['instance_type']))
